...
class ServicedObjectController extends AbstractController
{
    ...
    public function edit(Request $request, 
                        ...
                        File $files, 
                        ServicedObjectRepository $repo): Response
    {
        $objects_object = new ServicedObject();
        $file_object = new File();
        $objects_object->addFile($file_object);

        $em = $this->doctrine->getManager();
        //$myfiles = $em->getRepository("FileRepository.")->getFilesByIdObject($servicedObject->getId());
        //$myfiles = $repo->getFilesByIdObject($servicedObject->getId());
        //$responce = new JsonResponse($myfiles);
        //$responce->send();
        //exit();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            //$responce = new Response(json_encode($files, 512));
            //$responce->send();
            return new JsonResponse($servicedObject->getId());
            exit();
